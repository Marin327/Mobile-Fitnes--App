...
export default function CalorieCounter() {
  ...
  const addFood = () => {
    if (!food || !calories || isNaN(calories) || parseFloat(calories) < 0) {
      Alert.alert('Error', 'Please enter valid food and calorie values (calories cannot be negative)');
      return;
    }

    const newFood = { id: Date.now().toString(), food: food, calories: parseFloat(calories) };
  ...
  return (
    <View style={styles.container}>
      ...
      <TextInput
        ...
        autoFocus={true} // Фокус на първото поле
      />
      <TextInput
        style={styles.input}
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={styles.foodItem}>
            <Text style={styles.foodText}>{item.food} - {item.calories.toFixed(2)} kcal</Text>
          </View>
        )}
